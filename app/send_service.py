...
def create_eth_transaction(
    ...
    # Используем pending state.
    # Это важно, если у кошелька уже есть
    # неподтверждённая исходящая транзакция.
...
def sign_and_send_eth_transaction(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                "Failed to broadcast "
                f"Ethereum transaction: {str(exc)}"
            ),
        ) from exc
